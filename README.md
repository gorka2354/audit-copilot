...
**Grounding — cross-model judge 48%**: независимый судья (Ollama) проверяет, подтверждают
ли цитаты находку генератора (Anthropic) семантически. Faithfulness (структура: источник в
контексте) и grounding (семантика: цитата подтверждает вывод) меряют **разное** и намеренно
стоят рядом; 48% — честный таргет улучшения, а не самопохвала.
