...
def source_sha256(code: str) -> str:
    """sha256 исходника — привязывает фикстуру к конкретному контенту `.sol`."""
...
class ReplayAnalyzer:
    ...
    def analyze(self, source: SoliditySource) -> list[Finding]:
        ...
        # Провенанс: фикстура привязана к контенту .sol её sha. Расхождение = кто-то
        # правил исходник, но не перезаписал фикстуру → отказ вместо тихих старых чисел.
        recorded = data["source_sha256"]
        actual = source_sha256(source.code)
        if recorded != actual:
            raise ValueError(
                f"фикстура {source.path} записана для другого исходника "
                f"(sha {recorded[:12]} ≠ {actual[:12]}) — перезапиши: "
                "uv run python scripts/record_findings.py"
            )
        return [finding_from_dict(d) for d in data["findings"]]
